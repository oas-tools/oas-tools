...
export default () => {
    ...
    describe('\n    OAS-Params Middleware tests', () => {
        ...
        it('Should parse request body correctly taking defaults into account', async () => {
            await axios.post('http://localhost:8080/api/v1/oasParams/body/defaultFields', [{prop2: {prop31: [null, {}]}}] )
            .then(res => {
                let expected = {
                    params: {},
                    body: [{ prop2: { prop31: [{ prop311:"hello" }, { prop311:"hello" }], prop21: false}, prop1: 1 }]
                }
                assert.deepStrictEqual(res.data, expected);
            });
        });
