...
export class OASRouter extends OASBase {
  ...
  static async #loadControllers(oasFile, config) {
    ...
    try {
      if (config.endpoints) {
        // Load when annotations enabled

        /* Previous validations */
        const oasEndpoints = Object.entries(oasFile.paths).flatMap(
          ([path, pathObj]) =>
            Object.keys(pathObj).map(
              (method) => `${method.toUpperCase()} ${path}`
            )
        );
        const annEndpoints = Object.entries(config.endpoints).flatMap(
          ([path, pathArr]) =>
            pathArr.map((obj) => `${obj.method} ${commons.expressPath(path)}`)
        );
        ...
        if (missingMethod.length > 0)
          throw new Error(
            `No HTTP method specified for:\n > ${missingMethod.join("\n > ")}`
          );
        if (oasDiff.length > 0)
          logger.warn(
            `Missing Routing annotations(path, method) for:\n > ${oasDiff.join(
              "\n > "
            )}`
          );
        if (annDiff.length > 0)
          logger.warn(
            `Missing OAS declaration for:\n > ${annDiff.join("\n > ")}`
          );

        /* Import functions */
        await Promise.all(
          Object.entries(config.endpoints).map(async ([path, arr]) => {
            const tmp = {};
        ...
        // Load when annotations disabled
        ...
        await Promise.all(
          Object.entries(oasFile.paths).flatMap(([expressPath, obj]) => {
            const controllerName =
              obj["x-router-controller"] ??
              commons.generateName(expressPath, "controller");
            ...
            return Object.entries(obj)
              ...
              .map(async ([method, methodObj]) => {
                ...
                const opId =
                  ...
                  commons.generateName(expressPath, "function");
                const opControllerName =
                  methodObj["x-router-controller"] ?? controllerName;
                const path = commons.filePath(
                  config.controllers,
                  opControllerName
                );
                ...
                if (!path)
                  throw new errors.RoutingError(
                    `Controller ${opControllerName} not found`
                  );

                tmp[expressPath] = {
                  ...tmp[expressPath],
                  [method.toUpperCase()]: (await import(pathToFileURL(path)))[
                    opId
                  ],
                };
                ...
                if (!tmp[expressPath][method.toUpperCase()])
                  throw new Error(
                    `Controller ${path} does not have method ${opId}`
                  );
