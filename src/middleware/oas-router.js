 ...
 */
function checkResponse(res, oldSend, spec, method, url, content) {
  ...
  if (responseCodeSection == undefined) {
    ...
    if (config.strict == true) {
      logger.error(msg);
      content[0] = JSON.stringify({message: msg});
    ...
    if (responseCodeSection.hasOwnProperty('content')) { //if there is no content property for the given response then there is nothing to validate.
      ...
      console.log("//////////////////pre validate en checkResponse")
      validator.validate(data, validSchema, function(err, valid) {
        console.log("//////////////////dentro de validate en checkResponse")
        if (err) {
...
exports = module.exports = function(controllers) {
  ...
  return function OASRouter(req, res, next) {
    ...
    var parameters = spec.paths[url][method]['parameters'];
    if(parameters != undefined){
      var aux_path = requestedUrl.split('/').filter(Boolean);
      if(parameters.length != aux_path.length-1){
        logger.info("there are missing parameters!");
      }
      for(var i = 0; i<parameters.length;i++){
          res.locals[parameters[i].name] = aux_path[i+1]; //this is not needed anymore because petId is in req.params as routes were registered!
      }
    }
    ...
    res.send = function(data) { //intercept the response from the controller to check and validate it
      ...
      checkResponse(res, oldSend, spec, method, url, arguments);
    }

    //This OASRouter function should decide:
    //    WHICH operation from the controllers to execute and if this operation needs parameters:
    //    WHAT parameters are required for that (these parameters should be placed in the res.local object)
