 ...
 */
function checkResponse(res, oldSend, oasDoc, method, reqPath, content) {
  ...
  var responseCodeSection = oasDoc.paths[reqPath][method].responses[code]; //Section of the oasDoc file starting at a response code
...
/**
 * Returns the base path of the requested path (as appears on the oasDoc file) without any slashes
 * NO: Removes '/' from the requested url and returns a string representing the name (path) of the requested resource
 * @param {object} reqPath - Path containing '/' at the beggining.
 */
function nameOfPath(reqPath) {
  //var res = reqPath.toString().substring(1, reqPath.length).toString();
  var res = reqPath.split("/")[1];
  return res;
}
 ...
 */
function generateOpId(oasDoc, url, method) {
  if (oasDoc.paths[url][method].hasOwnProperty('operationId')) {
    return oasDoc.paths[url][method].operationId.toString(); // Use opID specified in the oas doc
  } else {
    return nameMethod(oasDoc.paths[url][method]) + nameOfPath(oasDoc.paths[url][method]); //if there is no opID in the oasDoc, then generate the identifier
  }
}
...
exports = module.exports = function(controllers) {
  ...
  return function OASRouter(req, res, next) {
    ...
    var reqPath = res.locals.reqPath; //requested path version on the oasDoc file of the requested url
    var requestedUrl = res.locals.requestedUrl;
    var method = req.method.toLowerCase();
    console.log("EN OAS-ROUTER valor de reqPath: " + reqPath)
    if (oasDoc.paths[reqPath][method].hasOwnProperty('x-router-controller')) { //oasDoc file has x-router-controllers property: use the controller specified there
      var controllerName = oasDoc.paths[reqPath][method]['x-router-controller'];
    } else if (existsController(controllers, generateName(requestedUrl))) { //oasDoc file doesn't have x-router-controllers property: use the standard controller name (autogenerated) if found
      var controllerName = generateName(reqPath);
    ...
    var opID = generateOpId(oasDoc, reqPath, method);
    ...
    res.send = function(data) { //intercept the response from the controller to check and validate it
      ...
      checkResponse(res, oldSend, oasDoc, method, reqPath, arguments);
    }
