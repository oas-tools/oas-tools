          ...
          for (path in paths) {
            for (var method in paths[path]) {
              if (paths[path][method].operationId != undefined) {
                var function_string = "module.exports." + paths[path][method].operationId + " = function " + paths[path][method].operationId + " (req, res, next) {\nDefault." + paths[path][method].operationId + "(req.swagger.params, res, next);\n};\n";
              }
              if (paths[path][method]['x-router-controller'] != undefined) {
                logger.debug("Write: " + paths[path][method].operationId)
                fs.appendFileSync(__dirname+'/nodejs-server-generated/controllers/' + paths[path][method]['x-router-controller'] + ".js", function_string);
              }
            }
          }
