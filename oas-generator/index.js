...
function getExpressVersion(oasPath) {
  var expressVersion = oasPath.replace(/{/g, ':').replace(/}/g, '');
        ...
        if (err) {
          ...
          for (path in paths) {
            for (var method in paths[path]) {
              if (paths[path][method].operationId != undefined) {
                ...
                opId = normalize(generateOperationId(method, path));
                logger.debug("Oas-doc does not have opearationId property for " + method.toUpperCase() + " - " + path + " -> operationId name autogenerated: " + opId);
              }
              if (paths[path][method]['x-router-controller'] != undefined) { //TODO: improve checking of x-swagger-router-controller
                ...
                logger.debug("Oas-doc does not have routing property for " + method.toUpperCase() + " - " + path + " -> controller name autogenerated: " + normalize_controllerName(controllerName));
              }
          ...
          var package_raw = {
            ...
            "dependencies": {
              "express": "^4.16.3",
              "js-yaml": "^3.3.0"
              //TODO: should have here oas-tools
            }
          };
